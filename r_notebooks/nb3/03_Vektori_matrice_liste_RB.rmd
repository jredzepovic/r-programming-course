...
Jer joj je tip znakovni niz, a taj je tip najjaci u tom vektoru.
...
Vektorizacija usporednih operatora vo je osnovni temelj tzv. *logičkog indeksiranja*, što je jedna od metoda indeksiranja koje ćemo upoznati u nastavku.

***

## Indeksni vektori

Osnovni princip odabira više elemenata odjednom je jednostavan - samo navodimo indekse traženih elemenata na odbrani način. R nudi tri osnovna načina indeksiranja:

- lokacijsko indeksiranje (engl. *integer- or location-based indexing*)
- uvjetno indeksiranje (engl.*conditional or boolean-based indexing*)
- imensko indeksiranje (engl.*label-based indexing*)

Koje indeksiranje ćemo odabrati ovisi o tome želimo li elementima pristupati ovisno o njihovoj lokaciji, imenu ili prema zadanom uvjetu. 

Svaki tip indeksiranja u suštini se svodi na korištenje **određenog tipa vektora kao parametra za operator indeksiranja**. Ovakav vektor se zbog svoje uloge naziva **indeksnim vektorom**.
...
*Lokacijsko indeksiranje* je poopćenje već upoznatog principa indeksiranja gdje navodimo redni broj elementa koji nas zanima. Ako želimo više elemenata, jednostavno navedemo njihove indekse "zapakirane" u numerički vektor.

Pokušajte riješiti sljedeći zadatak korištenjem odgovarajućih numeričkih vektora kao parametara indeksiranja.

***

## Zadatak 3.8- lokacijsko indeksiranje
...
## Zadatak 3.9 - lokacijsko indeksiranje (2)
...
## Zadatak 3.10 - lokacijsko indeksiranje i izmjena elemenata
...
# koristite lokacijsko indeksiranje i operator `<-`
# ispišite vektor `a`
a[2:8] <- 0
a
...
Ako smo pažljivo razmotrili rezultate dobivene kod primjera sa vektoriziranim usporednim operatorima onda smo mogli vrlo dobro naslutiti kako radi *uvjetno indeksiranje*. Princip je jednostavan - za indeksni vektor postavljamo **logički vektor iste duljine kao i vektor čije elemente želimo dohvatiti**. Elementi logičkog vektora određuju koje elemente zadržavamo (pozicije gdje se nalazi vrijednost `TRUE`) a koje odbacujemo (pozicije gdje se nalazi vrijednost `FALSE`).


***

## Zadatak 3.11 - uvjetno indeksiranje 
```{r}
x <- 1:10

# napravite logički vektor `y` duljine 10 sa proizvoljnom kombinacijom
# vrijednosti TRUE i FALSE
y <- c(T, T, T, F, F, T, F, T, T, T)
...
## Zadatak 3.12 - uvjetno indeksiranje (2)
```{r}
y <- seq(1, 100, 7)
...
*Imensko indeksiranje* radi na principu **eksplicitnog imenovanja elemenata koje želimo "zadržati"**. Da bi mogli koristiti ovakav tip indeksiranja moramo zadovoljiti nužan preduvjet - **elementi vektora moraju imati definirana "imena"**.
...
## Zadatak 3.13 - imensko indeksiranje
...
* i matrice i polja su zapravo jednodimenzionalni vektori kojima je dodan atribut dimenzionalnosti, a uz pomoć kojeg jezik R mapira naše naizgled višedimenzionalno indeksiranje u "stvarni" indeks elementa jednodimenzionalnog vektora
...
- prvo indeksiramo retke, a potom stupce, a indeksne vektore odvajamo zarezom
...
svastara$parniBrojevi <- seq(1,100, 2)
