...
#pogledajte strukturu podatkovnog okvira `titanic` uz pomoć funkcije `str`
...
## Funkcije `tbl_df` i `glimpse`

Osnovna ideja: olakšani rad sa podatkovnim okvirima i pregledniji ispis.

- funkcija `tbl_df` pretvara obični podatkovni okvir u objekt klase `tbl`
     - glavna prednost: kontrola ispisa, nema smrzavanja konzole kod slučajnog pokušaja ispisa velikih skupova podataka na zaslon

- funkcija `glimpse` je poboljšanje funkcije `str`
     - ispis strukture se prilagođava širini zaslona
...
```{r, eval = F}
# `df` je klase `tbl`, zelim vidjeti 50 redaka u ispisu
print(df, n = 50)
```

     

## ZADATAK 11.2 - funkcije `table_df` i `glimpse`

```{r}
#pozovite funkciju `tbl_df` nad tablicom `titanic`
#rezultat smjestite u istu varijablu
titanic <- tbl_df(titanic)
#pozovite funkciju `glimpse` nad tablicom `titanic`
glimpse(titanic)
```
...
filter(titanic, Survived == 1 & Sex == "male" & str_detect(Name, "George|Frank"))
...
filter(titanic, Sex == "female" & Pclass == 1) %>% arrange(desc(Age)) %>% top_n(5, Age)
```
...
top_n(titanic, 10, Age) %>% arrange(desc(Age)) %>% select(Name:Fare)
...
filter(titanic, is.na(Age)) %>% sample_n(10) %>% select(Name:Fare,Pclass,Survived) %>% arrange(Name)
```
...
titanic <- mutate(titanic, hadRelativeOnBoard = SibSp > 0 | Parch > 0)
...
filter(titanic, Age > 20 & Sex == "female" & Embarked == "S") %>% sample_n(10) %>% transmute(Name, Pclass, FareToday = paste0("$", round(Fare*23.85, 2))) %>% arrange(desc(Pclass))
...
mutate(titanic, FareCategory = ntile(Fare, 5)) %>% sample_n(20) %>% select(Name, Pclass, Fare, FareCategory) %>% arrange(FareCategory)
...
titanic <- mutate(titanic, EmbarkationPort = ifelse(Embarked == "S", "Southampton", ifelse(Embarked == "C", "Cherbourg", "Queenstown")))
...
data(hflights)
# pretvorite okvir u `hflights` u klasu `tbl`
...
```{r}
# stvorite varijablu `flight815` koja će sadržavati 
# retke iz tablice `hflights`vezane uz let broj 815
flight815 <- filter(hflights, FlightNum == 815)
# stvorite varijablu `grouped815` koja će sadržavati 
# retke iz `flight815` grupirane po mjesecu
...
summarise(flight815, mean(ArrDelay))
cat("------\n")
summarise(grouped815, mean(ArrDelay))
```
...
filter(hflights, CancellationCode == "B") %>% group_by(Month) %>% summarise(BadWeatherCancellations = n())
...
filter(hflights, Dest %in% c("LAX", "JFK", "LGA")) %>% group_by(DayOfWeek, Dest) %>% summarise(MeanArrDelay = round(mean(ArrDelay, na.rm = T), 2)) %>% arrange(desc(MeanArrDelay))

```


## ZADATAK 11.9 - složeni upiti (2)
...
mutate(hflights, ArrDelayCatId = ntile(ArrDelay, 10)) %>% group_by(ArrDelayCatId) %>% summarise(NFlights = n(), minDelay = min(ArrDelay), maxDelay = max(ArrDelay), MostFreqAirport = mostFreqValue(Dest))

# ponovite prethodni primjer ali umjesto funkcije 
# `ntile` isprobajte funkciju `cut`
mutate(hflights, ArrDelayCatId = cut(ArrDelay, 10)) %>% group_by(ArrDelayCatId) %>% summarise(NFlights = n(), minDelay = min(ArrDelay), maxDelay = max(ArrDelay), MostFreqAirport = mostFreqValue(Dest))
```
