...
rez <- l %>% lapply(function(x) c(x[1], x[length(x)])) %>% unlist %>% matrix(ncol = 2, byrow = T)
...
## ZADATAK 9.4 - podatkovni skup `studenti`
...
## ZADATAK 9.5 - prilagodba parametara učitavanja podatkovnog skupa
...
## ZADATAK 9.6 - funkcija `gather`


```{r}
# stvorite podatkovni okvir `ocjene` uz pomoć funkcije `gather` i okvira `studenti`
ocjene <- gather(studenti, "Predmet", "Ocjena", 4:10, na.rm = T, factor_key = T)
...
## ZADATAK 9.8 - podatkovni skup `auti`
...
## ZADATAK 9.10 - podatkovni skup `potrosackaKosarica`


```{r}
# učitajte podatke iz datoteke `potrosackaKosarica.csv` u podatkovni okvir imena `racuni`
racuni <- read.csv("potrosackaKosarica.csv", fileEncoding = "UTF-8", stringsAsFactors = F)
...
## ZADATAK 9.11 - pretvorba okvira u "dugi" format


```{r}
# pretvorite podatke okvira `racuni` iz "širokog" formata u "dugi"
racuniDugi <- gather(racuni, "Proizvod", "Kolicina", 2:21)
racuniDugi <- racuniDugi [racuniDugi$Kolicina != 0, 1:2]
racuniDugi <- racuniDugi[order(racuniDugi$racunID), ]
write.csv(racuniDugi, file = 'potrosackaKosaricaDugiFormat.csv', row.names = F, fileEncoding = "UTF-8")
...
## ZADATAK 9.12 - pretvorba okvira u "široki" format
...
racuniSiroki$Kolicina <- 1
racuniSiroki <- spread(racuniSiroki, "Proizvod", "Kolicina", fill = 0)
write.csv(racuniSiroki, file = 'potrosackaKosaricaSirokiFormat.csv', row.names = F, fileEncoding = "UTF-8")
head(racuniSiroki)
```
...
## ZADATAK 9.13 - podatkovni skup `odjeli`


```{r}
# čitajte podatke iz datoteke `odjeli.csv` u varijablu `odjeli`
odjeli <- read.csv("odjeli.csv", fileEncoding = "UTF-8", stringsAsFactors = F)
...
## ZADATAK 9.14 - funkcija `separate`
...
odjeli2 <- separate(odjeli, "Kvartal", c("Kvartal", "Godina"), "-", remove = T)
...
odjeli3 <- unite(odjeli2, "Kvartal", c("Kvartal", "Godina"), sep = "-", remove = T)
