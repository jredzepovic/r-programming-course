...
# uz pomoć funkcije `data.frame` i gornjih vektora stvorite podatkovni okvir `mjesto`
# koristite istu sintaksu kao kod stvaranja liste
# elemente liste nazovite imenima varijabli
mjesto <- data.frame(pbr=pbr,
                     nazivMjesta=nazivMjesta,
                     prosjPlacaKn=prosjPlacaKn,
...
- dvodimenzionalno indeksiranje uz pomoć indeksnih vektora (matrice!)
...
mjesto$mjesecniPrihod <- round(0.01*mjesto$prirez*(mjesto$prosjPlacaKn*0.1) * 
                                 mjesto$brojStanovnika*0.6 / 1e6,2)
...
- brisanje redaka i stupaca možemo obaviti dvodimenzionalnim indeksiranjem redaka i stupaca koje želimo "zadržati"
...
```{r}
# obrišite prvi redak i drugi stupac iz df12 metodom indeksiranja
df12 <- df12[2:4,c(1,3,4)]
df12
...
# inačice varijable `tlak` (funkcija `factor`!)
tlak.f <- factor(tlak)
...
# ali uz dodatni parametar `order = TRUE`
# pripazite da poredak kategorija odgovara ordinalnom rasporedu
tlak.f3 <- factor(tlak, levels=c("prenizak", "nizak", "normalan", "visok", "previsok"), order = TRUE)
