...
veci <- function(vector1, vector2) {
  stopifnot((is.numeric(vector1) && is.numeric(vector2)) && (length(vector1)==length(vector2)))
  rez <- ifelse(vector1>vector2, vector1, vector2)
  rez
}
...
ispisiParametre(1,2,3,4)

```

***

## Konstruktor S3 objekta
...
ispisi.Osoba <- function(osoba) {
  cat(paste("oib: ", osoba$oib, "prezime: ", osoba$prezime, "tezina: ", osoba$tezina))
}
...
Za kraj, prikažimo mogućnost R-a da nam izlista sve trenutno poznate implementacije neke generičke funkcije. Za to se jednostavno koristimo funkcijom `methods` kojoj proslijedimo ime dotične funkcije. Istom funkcijom možemo i provjeriti koje sve implementacije generičkih funkcija postoje za određenu klasu. Za to koristimo parametar `class` kojem prosljeđujemo naziv klase za koju tražimo spomenute implementacije.

***

## Zadatak 7.8 - funkcija `methods`
